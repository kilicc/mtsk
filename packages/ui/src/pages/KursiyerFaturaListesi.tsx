...
type ExtendedKursiyer = Kursiyer & {
  ...
  aday_no?: string;
  hesaplanan_fatura?: number;
  kesilen_fatura?: number;
  eksik_kalan_fatura?: number;
};
...
export default function KursiyerFaturaListesi() {
  ...
  const loadKursiyerler = async () => {
    try {
      ...
      const extendedData: ExtendedKursiyer[] = (data.length > 0 ? data : getMockKursiyerler()).map((k: any) => {
        const hesaplanan = Math.floor(Math.random() * 20000) + 10000;
        const kesilen = Math.floor(Math.random() * hesaplanan);
        const eksik = hesaplanan - kesilen;
        return {
          ...k,
          ...
          hesaplanan_fatura: hesaplanan,
          kesilen_fatura: kesilen,
          eksik_kalan_fatura: eksik,
        };
      });
  ...
  const applyFilters = (data: FilterData, source: ExtendedKursiyer[] = allKursiyerler) => {
    ...
    if (data.bakiye_tutari_min) {
      ...
      result = result.filter(k => (k.eksik_kalan_fatura || 0) >= min);
    }
    if (data.bakiye_tutari_max) {
      const max = parseFloat(data.bakiye_tutari_max);
      result = result.filter(k => (k.eksik_kalan_fatura || 0) <= max);
    }
    if (data.bakiyesi_olanlari_goster) result = result.filter(k => (k.eksik_kalan_fatura || 0) > 0);
  ...
  return (
    <div className="p-4 space-y-6" style={{ height: '100%', overflowY: 'auto' }}>
      ...
      {/* Filtreleme Bölümü - Bakiye Raporu ile aynı yapı, kısaltılmış */}
      {showFilters && (
        <div className="card">
          ...
          {activeFilterTab === 'f1' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-adi">Adı</label>
                <input
                  id="fatura-f1-adi"
                  type="text"
                  value={filters.adi}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-soyadi">Soyadı</label>
                <input
                  id="fatura-f1-soyadi"
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-tc">TC Kimlik No</label>
                <input
                  id="fatura-f1-tc"
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-tel">Cep Telefonu</label>
                <input
                  id="fatura-f1-tel"
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-mevcut-belge">Mevcut Belge</label>
                <select
                  id="fatura-f1-mevcut-belge"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-istenen">İstediği Sertifika</label>
                <select
                  id="fatura-f1-istenen"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-aday-min">Aday No (Min)</label>
                <input
                  id="fatura-f1-aday-min"
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-aday-max">Aday No (Max)</label>
                <input
                  id="fatura-f1-aday-max"
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-aday-durum">Aday No Durumu</label>
                <select
                  id="fatura-f1-aday-durum"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-genel-durum">Genel Durum</label>
                <select
                  id="fatura-f1-genel-durum"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-bakiye-turu">Bakiye Türü</label>
                <select
                  id="fatura-f1-bakiye-turu"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-bakiye-min">Bakiye Tutarı (Min)</label>
                <input
                  id="fatura-f1-bakiye-min"
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-f1-bakiye-max">Bakiye Tutarı (Max)</label>
                <input
                  id="fatura-f1-bakiye-max"
                  type="number"
                  step="0.01"
              ...
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="fatura-f1-bakiye-goster"
                  checked={filters.bakiyesi_olanlari_goster}
                  onChange={(e) => setFilters({ ...filters, bakiyesi_olanlari_goster: e.target.checked })}
                  className="w-4 h-4 text-gray-600"
                />
                <label className="ml-2 text-sm text-gray-700" htmlFor="fatura-f1-bakiye-goster">Bakiyesi Olanları Göster</label>
              </div>
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="fatura-f1-ayni-tc"
                  checked={filters.ayni_tc_olanlari_goster}
                  onChange={(e) => setFilters({ ...filters, ayni_tc_olanlari_goster: e.target.checked })}
                  className="w-4 h-4 text-gray-600"
                />
                <label className="ml-2 text-sm text-gray-700" htmlFor="fatura-f1-ayni-tc">Aynı TC Olanları Göster</label>
              </div>
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="fatura-f1-dogum-gunu"
                  checked={filters.bugun_dogum_gunu_olanlar}
                  onChange={(e) => setFilters({ ...filters, bugun_dogum_gunu_olanlar: e.target.checked })}
                  className="w-4 h-4 text-gray-600"
                />
                <label className="ml-2 text-sm text-gray-700" htmlFor="fatura-f1-dogum-gunu">Bugün Doğum Günü Olanlar</label>
              </div>
            </div>
          )}
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              {activeFilterTab === 's1' && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="fatura-s1-alan">Sıralama Alanı</label>
                    <select
                      id="fatura-s1-alan"
                    ...
                    >
                      ...
                      <option value="eksik_kalan_fatura">Eksik Kalan Fatura</option>
                    </select>
                  </div>
                  <div className="flex items-center">
                    <input
                      ...
                      className="w-4 h-4 text-gray-600"
                    />
                    <label className="ml-2 text-sm text-gray-700" htmlFor="fatura-s1-ters">Ters</label>
                  </div>
                </div>
              )}
            </div>
            ...
            <table className="table text-sm">
              <thead>
                <tr>
                  ...
                  <th>Hesaplanan Fatura</th>
                  <th>Kesilen Fatura</th>
                  <th>Eksik Kalan Fatura</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  paginatedKursiyerler.map((kursiyer, index) => (
                    ...
                    >
                      ...
                      <td className="text-amber-600 font-bold">
                        {formatCurrency(kursiyer.hesaplanan_fatura || 0)}
                      </td>
                      <td className="text-green-600 font-bold">
                        {formatCurrency(kursiyer.kesilen_fatura || 0)}
                      </td>
                      <td className="text-red-600 font-bold">
                        {formatCurrency(kursiyer.eksik_kalan_fatura || 0)}
                      </td>
                    </tr>
                  ))
                )}
